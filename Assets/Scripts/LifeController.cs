...
public class LifeController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (!moveToTarget || moveDelay <= 0) this.transform.position += direction.normalized * lifeSpeed * Time.deltaTime;
        if (moveToTarget)
        {
            moveDelay -= Time.deltaTime;
            if (direction.magnitude < 1f)
            {
                Core.AtualizarIncrementoVida();
